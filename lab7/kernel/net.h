...
#define MBUF_CSUM_OFLD_ENABLE  1
#define MBUF_CSUM_OFLD_TCP     2
struct mbuf {
...
struct arpq {
  struct arpq *next; // the next ip miss in the list
  uint32 ip;      // the remote IPv4 address
  struct mbufq txq;  // a queue of packets waiting to be transmitted
};
...
struct sock {
  ...
  uint8  type;       // sock type
  uint32 raddr;      // the remote IPv4 address
  uint16 lport;      // the local UDP port number
  uint16 rport;      // the remote UDP port number
  struct spinlock lock; // protects the rxq
  struct mbufq rxq;  // a queue of packets waiting to be received
  struct tcp_cb tcpcb; // only use in tcp sock 
};
