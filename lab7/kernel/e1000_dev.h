...
struct tx_desc
{
  ...
  uint16 length;
  uint8 cso; // Checksum Offset, indicates where to insert a TCP checksum if this mode is enabled
  uint8 cmd; // Command field
  uint8 status;
  uint8 css; // Checksum Start, where to begin computing the checksum
  uint16 special;
};
...
#define E1000_RXDW (1 << 7)
#define E1000_TXDW (1 << 0)
