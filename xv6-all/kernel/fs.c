...
iget(uint dev, uint inum)
{
  ...
  ip->ref = 1;
  ip->valid = 0;
  // avoid conflict when inode reused by procfs
...
dirlookup(struct inode *dp, char *name, uint *poff)
{
  uint off, inum;
  struct dirent de;
  int dev_dir = IS_DEV_DIR(dp);
  if(dp->type != T_DIR && !dev_dir)
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size || dev_dir; off += sizeof(de)){
    if(dev_dir){
      if(devsw[dp->major].read(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
        return 0;
    }else{
    ...
    if(namecmp(name, de.name) == 0){
      ...
      if (ip->valid == 0 && dev_dir && devsw[dp->major].inode_read) {
        devsw[dp->major].inode_read(ip);
      }
      return ip;
    }
  }
...
dirlink(struct inode *dp, char *name, uint inum)
{
  ...
  struct dirent de;
  struct inode *ip;
  // Check not from device fs
  if (dp->type == T_DEVICE) return -1;
